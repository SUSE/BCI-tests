        ...
        DerivedContainer(
            **_stunnel_kwargs,
            ...
            extra_environment_variables={
                ...
                "STUNNEL_SERVICE_NAME": "wikipedia_dot_org",
                "STUNNEL_ACCEPT": f"0.0.0.0:{_CTR_PORT}",
                "STUNNEL_CONNECT": "wikipedia.org:80",
            },
        ),
        marks=_marks,
...
def test_http_tunnel_to_suse_dot_com(container: ContainerData) -> None:
    """Simple smoke test of stunnel wrapping the HTTP connection to
    wikipedia.org with TLS.
    ...
    def get_request_to_stunnel(route: str = "") -> requests.Response:
        resp = requests.get(
            ...
            verify=_SERVER_PEM,
            timeout=5,
            # wikipedia.org tries to redirect to TLS version, but we don't want
...
@pytest.mark.parametrize(
    "container, log_level",
    [(_create_http_forward_ctr("6"), 6), (_create_http_forward_ctr("1"), 1)],
    indirect=["container"],
)
