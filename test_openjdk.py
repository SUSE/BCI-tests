...
class MyTester:
    ...
    def get_container(self):
        subprocess.check_call(
            ["echo", "woot", "|", "wall", "&&", "docker", "pull", self.container_url]
        )
        docker_id = (
            subprocess.check_output(
                ["docker", "run", "-d", "-it", self.container_url, "/bin/sh"]
            )
            .decode()
            .strip()
        )
...
class TestWithDifferentJDKs:
    jdks = [
        (
            "11",
            "registry.opensuse.org/home/fcrozat/matryoshka/containerfile/opensuse/openjdk:11",
        ),
        (
            "16",
            "registry.opensuse.org/home/fcrozat/matryoshka/containerfile/opensuse/openjdk:16",
        ),
    ]
    ...
    def test_jdk_version(self, tester):
        ...
        assert "openjdk {}".format(tester.jdk_version) in host.check_output(
            "java --version"
        )
